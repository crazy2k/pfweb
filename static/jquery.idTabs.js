...
idTabs = $.idTabs = function(tabarea,options,data){
  // Settings
  var e, tabs, test=$(), meta = $.metadata?$(tabarea).metadata():{}, //metadata
  ...
  tabs.each(function(){ //add items
    ...
    if(test.length) s.items.push(test[0]);
  });
  s.items.hide();  //hide items
  ...
  || type(s.start) == "String" && (test=tabs.filter("[href=#"+s.start+"]")).length
